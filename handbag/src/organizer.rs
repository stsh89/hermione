...
impl Organizer {
    ...
    pub fn get_command(
        ...
        workspace_id: &Id,
        command_id: &Id,
    ) -> OrganizerResult<&Command> {
        self.get_workspace(workspace_id)?
            .get_command(command_id)
    }

    pub fn get_workspace(&self, id: &Id) -> OrganizerResult<&Workspace> {
        self.workspaces.get(id.raw()).ok_or(OrganizerError::workspace_not_found(id))
    }

    pub fn get_workspace_mut(&mut self, id: &Id) -> OrganizerResult<&mut Workspace> {
        self.workspaces.get_mut(id.raw()).ok_or(OrganizerError::workspace_not_found(id))
    }
