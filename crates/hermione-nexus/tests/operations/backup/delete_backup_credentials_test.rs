...
use hermione_nexus::operations::DeleteBackupCredentialsOperation;

#[derive(Default)]
struct DeleteBackupCredentialsTestCase {
    storage: InMemoryStorage,
    operation: Operation<()>,
}

impl TestCase for DeleteBackupCredentialsTestCase {
    ...
    fn inspect_operation_results(&self, expectations: Table) {
        ...
        support::assert_backup_credentials_count(&self.storage, expected_count);
    }
...
#[test]
fn it_deletes_notion_backup_credentials() {
    let mut test_case = DeleteBackupCredentialsTestCase::default();
